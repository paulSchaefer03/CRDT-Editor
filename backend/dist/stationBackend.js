...
app.get('/stationen', (req, res) => {
    ensureDir(BASIS);
    const stationen = fs.readdirSync(BASIS).filter((f) => fs.statSync(path.join(BASIS, f)).isDirectory());
    res.json(stationen);
});
// GET /dokumente/:station – Liste aller Dokumente in einer Station
app.get('/dokumente/:station', (req, res) => {
...
app.post('/dokumente/:station/:name', (req, res) => {
    ...
    if (fs.existsSync(datei))
        return res.status(200).json({ info: 'Datei existiert bereits' });
...
app.post('/stationen/:name', (req, res) => {
